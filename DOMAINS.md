...
- **定义**：每需求一份的一次性执行产物，实现开始时创建，归档后移入 `docs/tasks/archive/`。落 `docs/tasks/<需求名>.md`（不归属任何限界上下文；跨上下文需求共用一份，目标分别引用各 spec）。只承载六件事：需求目标（引用 spec）、足迹（本任务触及的 git 仓库与计划触碰清单，供他方撞车预判求交）、从 spec 派生的验收清单、每条的验证状态（验证手段 + 结果）、范围外问题（顺手发现、本次不修）、评审自检（第 6 步派评审前的维度逐项结论）。SDD 执行状态的唯一交接介质：任何接手者只读此文件与 spec/design 即可续写实现。任务拆分留在会话内，不落盘。另锚定执行位置：基线分支与提交区间（起终点 commit），供评审圈定范围与归档后回溯。
- **反例**：≠ spec/design（持久真相文档 vs 一次性消耗品，无"与事实脱节"问题）；≠ 合入（见下）；≠ 交接文档（会话接力、不进版本库）。
- **消费者**：implement 第 2 步模板、提交区间与撞车预判（他方足迹）、第 6 步评审自检小节；code-review 第 1 步变更范围圈定；debug 第 1 步（任务文件结论只当线索）。

### 交接文档（handoff file）

- **定义**：会话间一次性接力产物。落 `{本会话临时目录}/handoff/<yyyyMMdd>-<topic>.md`，不进版本库。临时目录取本会话环境声明的临时工作目录（各 harness 不同，不写死）。让新会话只读此文件即可开工，不查询源会话。承载：项目路径、目标、已完成、线索（非本会话根因）、复现手段、下一步、相关路径（任务文件/spec 等只写路径）。
- **反例**：≠ 任务文件（SDD 执行状态、进 `docs/tasks/`；交接文档是会话接力）。
- **消费者**：handoff。

### 归档

- **定义**：任务文件从 `docs/tasks/` 移入 `docs/tasks/archive/`。条件：验收通过、声称未失真、最近一次完整评审无未处理发现。scoped 可合入不够。
- **反例**：≠ 合入。
- **消费者**：implement 第 7 步。

### 合入

- **定义**：任务分支合入任务文件记录的基线分支并 squash 收口，等价比对通过后移除 worktree（含本地任务分支）。合入前最近一次必须是工作分支相对基线的完整评审且无未处理发现；没有则先做完整评审。
- **反例**：≠ 归档。
- **消费者**：implement「合入」节。
...
- **反例**：≠ spec（断言信源 vs 派生视图）；≠ 任务文件（持久手册 vs 一次性消耗品）；≠ ARCHITECTURE.md（单上下文前端接口实操视图 vs 项目级架构事实）。
