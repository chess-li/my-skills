...
- **定义**：按限界上下文组织的持久派生文档，持续声称该上下文当前对外对接面的事实——接口契约（endpoint、入参出参、错误码）与联调步骤，读者是前端/下游/调用方。内容从 spec 接口断言与代码派生：断言信源仍在 spec，对接文档不新立断言。落 `docs/contexts/<限界上下文>/<限界上下文>-对接.md`，与 spec/design 共置。触及对接面的任务收尾时由 implement 刷新；未触及的任务不触发。
- **反例**：≠ spec（断言信源 vs 派生视图）；≠ 任务文件（持久手册 vs 一次性消耗品）；≠ ARCHITECTURE.md（单上下文接口实操视图 vs 项目级架构事实）。
- **消费者**：implement「完成与归档」步。
