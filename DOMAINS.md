...
- **定义**：每需求一份的一次性执行产物，实现开始时创建，归档后移入 `docs/tasks/archive/`。落 `docs/tasks/<需求名>.md`（不归属任何限界上下文；跨上下文需求共用一份，目标分别引用各 spec）。只承载四件事：需求目标（引用 spec）、从 spec 派生的验收清单、每条的验证状态（验证手段 + 结果）、范围外问题（顺手发现、本次不修）。执行的唯一交接介质：任何接手者只读此文件与 spec/design 即可续写。任务拆分留在会话内，不落盘。另锚定执行位置：基线分支与提交区间（起终点 commit），供评审圈定范围与归档后回溯。
- **反例**：≠ spec/design（持久真相文档 vs 一次性消耗品，无"与事实脱节"问题）；≠ 合入（见下）。
...
- **定义**：任务分支合入任务文件记录的基线分支，移除 worktree（含本地任务分支），并 squash 收口。合入前最近一次必须是工作分支相对基线的完整评审且无未处理发现；没有则先做完整评审。
- **反例**：≠ 归档。
- **消费者**：implement「合入」节。
