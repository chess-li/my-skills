...
- **定义**：每需求一份的一次性执行产物，实现开始时创建，完成后移入 `docs/tasks/archive/`。落 `docs/tasks/<需求名>.md`（不归属任何限界上下文；跨上下文需求共用一份，目标分别引用各 spec）。只承载四件事：需求目标（引用 spec）、从 spec 派生的验收清单、每条的验证状态（验证手段 + 结果）、范围外问题（顺手发现、本次不修）。执行的唯一交接介质：任何接手者只读此文件与 spec/design 即可续写。任务拆分留在会话内，不落盘。另锚定执行位置：基线分支与提交区间（起终点 commit），供评审圈定范围与归档后回溯。
...
- **消费者**：implement「完成与归档」步（增量刷新）；frontend-api-doc（存量生成/重建）。
