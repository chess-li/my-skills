...
- **策略层（skill-iteration-review）**：证据判据、取证流程、升格闸、四步工作流——判断的载体
...
新概念先问归属层，不问"要不要新 skill"。捕获不是 skill（没有模型触发场景）；创建由 skill-creator + 原理层值守；仅当某职责长出独立触发场景（如每周自动巡检证据日志的定时审计），才允许拆出独立 skill——且优先 user-invoked 或定时触发，不付常驻成本。
